...
class SDRemoteClient:
    ...
    def generate_image(
        ...
        try:
            ...
            # 构建完整命令
            if python_command.startswith("cd") or "&& " in python_command:
                # 如果 python_command 已经包含 cd 命令或多个命令
                # 格式: "cd /root/xxx && python" 或 "cd /root/xxx && /usr/bin/python3"
                command = f'{python_command} scripts/img2img.py'
            else:
                # 普通的 python 或 python3 命令（不包含 cd）
            ...
            # 6. 执行命令
            print("\n[3/4] 执行生成命令")
            stdout, stderr, exit_code = self.execute_command(command, timeout=600)
...
def main():
    ...
    # 提示词
    PROMPT = "A new item created by fusing materials into one coherent design"
